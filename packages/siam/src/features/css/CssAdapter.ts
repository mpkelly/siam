...
export const Projects:ProjectDescriptor[] = [
	{name:"CSS Variables", description: "Generate a single CSS file contain only the system tokens as CSS variables."}
]
			...
			Object.keys(value).forEach(token => {
				...
				if (isObject(tokenValue)) {
					...
					Object.keys(tokenValue).forEach((breakpoint) => {
						const variable = createVariable(property, token, (tokenValue as any)[breakpoint]);
